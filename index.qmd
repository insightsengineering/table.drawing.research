...
After testing each feature with different rendering/output engines, the
following changes were discovered/requested for change:

#### Features Requests for {gt} --\> gtsave(.docx)
...
-   gt::fmt_markdown(columns = label) returns XML code in .docx file
    with indentation.
